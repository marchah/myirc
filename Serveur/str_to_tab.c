...
** Last update Wed Apr 18 17:21:45 2012 hugo marchadier
*/
...
static char		*copy_next_word(char *s, unsigned int *pos)
{
  ...
  word = xmalloc_serveur(sizeof(*word) * (len + 1));
  word[len] = '\0';
...
char			**str_to_tab_serveur(char *s)
{
  unsigned int		nb_words;
  char			**tab;
  unsigned int		i;
  ...
  tab = xmalloc_serveur(sizeof(*tab) * (nb_words + 1));
  tab[nb_words] = NULL;
  i = 0;
  cursor = 0;
  while (i < nb_words)
